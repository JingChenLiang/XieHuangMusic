...
package xiehuang.com.android.xiehuangmusic.fm;
...
//gionee jiaoyuan 20130909 add  for CR00889510 begin
//gionee jiaoyuan 20130909 add  for CR00889510 end
    ...
    private class FmServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_TOFMSERVICE_POWERDOWN.equals(action)||(SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE.equals(command))) {
                ...
                Log.e(TAG,"airplane enable state is:" + enabled);
                       if(enabled){
			    showToast(getString(R.string.stop_fm_while_airplane_on));
                	    exitFm();
                            stopSelf();
                       //Gionee add for ximalaya
                      sendBroadcast(mFMserviceExit);
...
Log.d(TAG, "exitFm2");
    ...
    private void showToast(CharSequence text) {
        ...
        mToast.setText(text);
        mToast.show();
        Log.v(TAG, "showToast: toast = " + text);
    };
    //gionee jiaoyuan 20130909 add  for CR00889510 end
    /**
...
//Gionee add for ximalaya begin1
   public boolean isPowerUp(){
Log.d(TAG, "addlog mPowerStatus is "+mPowerStatus);
       if(mPowerStatus == POWER_UP){
           return true;
        }
    ...
    private final OnAudioFocusChangeListener mAudioFocusChangeListener =
            new OnAudioFocusChangeListener() {
                ...
                public void onAudioFocusChange(int focusChange) {
                 ...
                 Log.w(TAG, "fm auto shutdown for loss focus");
                 showToast(getString(R.string.fm_shutdown_automatically));//add tips
                 // need remove all messages, make power down will be execute
                 mFmServiceHandler.removeCallbacksAndMessages(null);
                 exitFm();
...
//Gionee modify 20160317 fengyao for CR01652558 end
 ...
 Log.d(TAG, "mPowerStatus is "+mPowerStatus+" mPausedByTransientLossOfFocus is:"+mPausedByTransientLossOfFocus);
 ...
 Log.d(TAG, "I'm msgid_swich_anteana in service" );//fengyao
                    bundle = msg.getData();
...
Log.d(TAG, "I'm msgid_swich_anteana in handle powerup" );//fengyao
        boolean isPowerUp = false;
        boolean isSwitch = true;
    ...
    */
    ...
    public boolean isActivityForeground() {
        ...
        Log.d(TAG, "isActivityForeground() return " + bRet);
        return bRet;
    }
...
//Gionee <bug> 20160223 fengyao add for CR01639203 end
    ...
    private IFmRadioService.Stub mAIDLBinder = new IFmRadioService.Stub() {
        ...
        public boolean closeDevice() {
        	return FmService.this.closeDevice();
        }
        ...
        public boolean powerUp(float frequency) {
            ...
            Log.d(TAG, "performance test from aidl. service handler power up start:" + time);
...
Log.d(TAG, "addlog firstplay"+FmUtils.isFirstTimePlayFm(mContext) );//fengyao
            ...
            Log.d(TAG, "performance test from aidl. service handler power up end:" + time);
 Log.d(TAG, "addlog isPowerup"+isPowerup );           
            return isPowerup;
        }
...
//Gionee add for ximalaya
        ...
        public void enablePSRT(boolean enable) {
            Log.d(TAG, "FmService.enablePSRT: " + enable);
        ...
        public void enableAF(boolean enable) {
            Log.d(TAG, "FmService.enableAF: " + enable);
            mIsAFEnabled = enable;
        }

        public void enableTA(boolean enable) {
            Log.d(TAG, ">>> FmService.enableTA: " + enable);
            Log.d(TAG, "<<< FmService.enableTA");
        }

        public boolean isPSRTEnabled() {
            Log.d(TAG, "FmService.isPSRTEnabled: " );//+ mIsPSRTEnabled);
            //return mIsPSRTEnabled;
            return true;
        }

        public boolean isAFEnabled() {
            Log.d(TAG, "FmService.isAFEnabled: " + mIsAFEnabled);
            return mIsAFEnabled;
        }

        public boolean isTAEnabled() {
        	Log.d(TAG, ">>> FmService.isTAEnabled");
        	return false;
        }
    ...
    public boolean onUnbind(Intent intent) {
        if(isActivityForeground(MMI_PACKAGE)){
            Log.d(TAG, ">>>stopSelf, startId = "+mServiceStartId);
